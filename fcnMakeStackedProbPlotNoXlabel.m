function fcnMakeStackedProbPlotNoXlabel(pstate1,pstate2,pstate3,pstate4,time,ylabelNo); 
...
hold on; patch(xx,yy,[1 0.6 0],'facealpha',0.5); 

xx=[x fliplr(x)]; yy=[y(2,:) fliplr(y(3,:))];
hold on; patch(xx,yy,[1 0 0],'facealpha',0.5); 

xx=[x fliplr(x)]; yy=[y(3,:) fliplr(y(4,:))];
hold on; patch(xx,yy,[1 0 0],'facealpha',1); 
...
if ylabelNo
    ...
    ylabel('State probabilities','fontsize',12)
end
set(gca,'fontsize',12)
